...
def test_triple(model, cfg):
    with Timer('1.prepare Dataset'):
        ...
        for i, (feature, center_idx, mask, idx, nodes_label, adj, adj_local) in enumerate(data_loader):
            ...
            print('loss_sum:{:.8f},loss1:{:.8f},loss2:{:.8f},{}/{}'.format(loss_sum.item(), loss1.item(), loss2.item(), i,
                                                                     int(dataset.inst_num / cfg.batch_size)))

        np.save("./data/Out_feature/cluster_feature.npy", final_feature)
